...
const CountingCrow = () => {
  const [cards, setCards] = useState<string[]>(Array(7).fill(""));
  ...
  const handleSubmit = async () => {
    ...
    if (filledCards.length !== 7) {
      toast.error("Molim te unesi sve 7 karata za Counting Crow spread");
      return;
    }
    ...
    try {
      const response = await fetch("http://localhost:8000/counting-crow", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ cards: filledCards }),
      });
